...
/**
* @brief Metric to get Ability network configuration.
*/
DECLARE_EXEC_NETWORK_METRIC_KEY(AB_CONFIG_OPENVINO_VER, std::string);
DECLARE_EXEC_NETWORK_METRIC_KEY(AB_CONFIG_ABGRAPH_VER, std::string);
DECLARE_EXEC_NETWORK_METRIC_KEY(AB_CONFIG_RESOLUTION, std::tuple<unsigned int, unsigned int>);
#endif
}  // namespace Metrics
