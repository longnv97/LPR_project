...
#define ABILITY_CUSTOMIZED_SYMBOL              (0xABBA)

typedef struct _Ability_CustInferData_ {
    unsigned short  Header; 
    unsigned int    DataSize;
    char*           DataPtr;
    unsigned short  Tailer;
} Ability_CustInferData,*pAbility_CustInferData;
#endif
...
#ifdef ABILITY_CUSTOMIZED
    void InferEx(const std::string& name,AbCustomInferenceData* requestData) {
        Ability_CustInferData CustInferData = {0};
        CustInferData.Header    = ABILITY_CUSTOMIZED_SYMBOL;
        CustInferData.Tailer    = ABILITY_CUSTOMIZED_SYMBOL;
        CustInferData.DataSize  = requestData->DataSize;
        CustInferData.DataPtr   = requestData->DataPtr;
        auto input_data = this->GetBlob(name)->buffer().as<PrecisionTrait<Precision::U8>::value_type *>();
        memcpy(input_data, &CustInferData, sizeof(CustInferData));
        CALL_STATUS_FNC_NO_ARGS(Infer);
    }
#endif
    /**
...
#ifdef ABILITY_CUSTOMIZED
    void StartAsyncEx(const std::string& name,AbCustomInferenceData* requestData) {
        Ability_CustInferData CustInferData = {0};
        CustInferData.Header    = ABILITY_CUSTOMIZED_SYMBOL;
        CustInferData.Tailer    = ABILITY_CUSTOMIZED_SYMBOL;
        CustInferData.DataSize  = requestData->DataSize;
